      ...
      function (data) {
        ...
        data.forEach((element) => {
          html += `<tr>
                    ...
                    <td class="text-center">
                        <a class="btn btn-sm btn-alt-secondary btn-delete-assignment" data-bs-toggle="tooltip" aria-label="Delete" data-bs-original-title="Delete" data-id="${
                          element["manguoidung"]
                        }">
  ...
  $(document).on("change", "#giang-vien", function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        let data = response;
        data.forEach((element) => {
          $(`input:checkbox[value=${element["mamonhoc"]}]`).attr(
            "checked",
            "checked"
          );
        });
        ...
        if (response) {
          ...
          setTimeout(function () {
            Dashmix.helpers("jq-notify", {
              ...
              message: "Phân công chưa thành công!",
            });
          }, 10);
        }
        loadAssignment();
